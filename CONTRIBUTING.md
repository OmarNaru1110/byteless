# Contributing to byteless

Thank you for your interest in contributing to **byteless**! We welcome contributions from the community to help make this project better. Whether it's reporting/fixing bugs, improving documentation, or adding new features, your help is appreciated.
...
- **Wails CLI**: Install the Wails command-line tool.
  ...
  ```
- **FFmpeg**: [ffmpeg](https://www.ffmpeg.org/download.html) and ffprobe are required at runtime. See the README section on dependency discovery for where byteless looks for them.
...
2.  **Clone the Repository**: Clone your forked repository to your local machine.
    ```bash
    git clone https://github.com/YOUR_USERNAME/byteless.git
    cd byteless
    ```
...
The backend ships with unit tests covering the domain, command, config, and util packages:
...
- **`/app.go`**: Contains the main application logic and bridge methods exposed to the frontend (loading videos, compression, encoder list, folder pickers, etc.).
- **`/main.go`**: Entry point of the application.
- **`/frontend`**: The React frontend application.
  - **`/src`**: Source code for the UI components.
  - **`/wailsjs`**: Auto-generated Go bindings for the frontend.
- **`/internal`**: Internal Go packages.
  - **`/builder`**: Fluent command builders for ffmpeg and ffprobe.
  - **`/command`**: Executable command implementations (two-pass encode, video details probing) and event emission.
  - **`/config`**: Path resolution for the ffmpeg/ffprobe binaries.
  - **`/domain`**: Core data models (Video, OutputVideo, encoder definitions).
  - **`/util`**: Shared helpers (size/bitrate math, name generation, platform utilities).
  - **`/testhelper`**: Test helpers.
- **`wails.json`**: Project configuration for Wails.

## Code Style

- **Go**: Please ensure your Go code is formatted using `gofmt`, and run `go vet ./...` and `go test ./...` before submitting.
- **Frontend**: Follow the existing coding style in the `frontend` directory. A formatter is available:
  ```bash
  cd frontend
  npm run format
  ```
